...
use super::{CellId, CellLookup, SpatialHashFilter, SpatialHashSystem};
...
///
/// Partitions are built on top of [`CellLookup`], only dealing with [`GridCell`]s. For
/// performance reasons, partitions do not track grid occupancy at the `Entity` level. Instead,
/// partitions are only concerned with which cells are occupied. To find what entities are
/// present, you will need to look up each of the partition's [`CellId`]s in the [`CellLookup`]
#[derive(Resource)]
...
mod private {
    use crate::{CellCoord, CellHashSet, CellId, GridPrecision};
    use bevy_ecs::prelude::*;
    use bevy_platform::prelude::*;
    ...
    impl Partition {
        ...
        fn compute_min(&mut self) {
            if let Some(min) = self.iter().map(CellId::coord).reduce(|acc, e| acc.min(e)) {
                ...
                self.min = CellCoord::ONE * GridPrecision::MAX;
            }
        }
        ...
        fn compute_max(&mut self) {
            if let Some(max) = self.iter().map(CellId::coord).reduce(|acc, e| acc.max(e)) {
                ...
                self.min = CellCoord::ONE * GridPrecision::MIN;
            }
        }
    }
}
